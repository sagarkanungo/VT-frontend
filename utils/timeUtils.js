...
export const getTimeSettings = async () => {
  ...
  if (timeSettingsCache && cacheTimestamp && (now - cacheTimestamp) < CACHE_DURATION) {
    console.log('📋 Using cached time settings:', timeSettingsCache);
    return timeSettingsCache;
  }
  
  try {
    // Use the public endpoint for all users
    console.log('🌐 Fetching time settings from public API...');
    const res = await apiClient.get('/api/time-settings');
    
    if (res.data && typeof res.data === 'object') {
      timeSettingsCache = res.data;
      cacheTimestamp = now;
      console.log('✅ Successfully loaded time settings from API:', timeSettingsCache);
      return timeSettingsCache;
    } else {
    ...
    const defaultSettings = { enabled: false, startTime: '09:00', endTime: '15:00' };
    console.log('📋 Using default time settings (API failed):', defaultSettings);
...
export const isTransactionAllowed = async () => {
  try {
    ...
    console.log('🔍 Current time settings:', settings);
    
    // If time restrictions are disabled, allow all transactions
    if (!settings.enabled || !settings.startTime || !settings.endTime) {
      console.log('✅ Time restrictions disabled or invalid settings - ALLOWING transactions');
    ...
    try {
      ...
      console.log(`📅 Allowed time window: ${settings.startTime} to ${settings.endTime} (${startTimeInMinutes} to ${endTimeInMinutes} minutes from midnight)`);
      
      const isAllowed = currentTimeInMinutes >= startTimeInMinutes && currentTimeInMinutes <= endTimeInMinutes;
      
      if (isAllowed) {
        console.log('✅ TRANSACTION ALLOWED - Current time is within allowed window');
      } else {
        console.log('❌ TRANSACTION BLOCKED - Current time is outside allowed window');
      }
...
export const clearTimeSettingsCache = () => {
  ...
  cacheTimestamp = null;
  console.log('🧹 Cleared time settings cache');
};
...
const AdminDashboard = () => {
    ...
    try {
      ...
      const usersWithBalance = await Promise.all(
        normalUsers.map(async user => {
          try {
            const balanceRes = await apiClient.get(`/api/user/${user.id}/balance`).catch(() => ({ data: { balance: 0 } }));
            const balance = balanceRes.data.balance;
            const numericBalance = typeof balance === 'string' ? parseFloat(balance) : (balance || 0);
            return { ...user, balance: isNaN(numericBalance) ? 0 : numericBalance };
          } catch {
  ...
  const handleEditChange = e => {
    ...
    if (name === 'id_document') setEditFormData(prev => ({ ...prev, id_document: files[0] }));
  ...
  return (
    <div className="admin-users-container">
      ...
      <div className="table-responsive">
            ...
            {currentUsers.length === 0 ? (
              ...
              <tr key={u.id}>
                ...
                <td>
                  {u.id_document ? (
                    <img
                      src={`${BACKEND_URL}/${u.id_document.replace(/\\/g, '/')}`}
                      alt="ID Document"
                      style={{ width: '80px', cursor: 'pointer', borderRadius: '4px' }}
                      onClick={() => window.open(`${BACKEND_URL}/${u.id_document.replace(/\\/g, '/')}`, "_blank")}
        ...
        <Pagination currentPage={currentPage} totalItems={filteredUsers.length} itemsPerPage={itemsPerPage} onPageChange={handlePageChange} />
      </div>
